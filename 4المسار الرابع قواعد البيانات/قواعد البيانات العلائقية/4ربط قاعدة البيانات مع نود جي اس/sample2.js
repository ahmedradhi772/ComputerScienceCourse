...
//----------------------------------------------------------- INSERT Data ----------------------------------------------------------------//
// اضافة البيانات للجدوال بشكل مبسط
/*db.run('INSERT INTO Comments (Content, Name, Date, ArticleID) VALUES ("مرحبًا مقالة رائعة", "حسن", "2024", 9999)',
function(err) {
  if (err) {
    return console.log(err.message);
  }
...
// اعرض اسم المقال وسنة تاريخ المقال وعدد التعليقات لكل مقال
...
/*db.run('UPDATE Comments SET Content = "للأسف  مقالة لم تعجبني" WHERE CommentID = 1',function(err){
    if(err){
        return console.log(err.message);
    }
...
/*db.run('DELETE FROM Comments WHERE CommentID = 1',function(err){
    if(err){
        return console.log(err.message);
    }
...
db.all("SELECT Comments.Name,Comments.Content,Articles.ArticleName, Articles.date FROM Comments INNER JOIN Articles on Comments.ArticleID = Articles.ArticleID ", function(err,table){
    if(err) return console.log(err.message);
    console.log(table)
});
...
// مشروع الوحدة
/*db.all(`SELECT Comments.Name, Comments.Content, Articles.ArticleName, Articles.Date
FROM Comments JOIN Articles 
ON Comments.ArticleID = Articles.ArticleID;`, function (err, table){
    if(err) {
        return  console.error(err.message);
    }
    console.log(table);
